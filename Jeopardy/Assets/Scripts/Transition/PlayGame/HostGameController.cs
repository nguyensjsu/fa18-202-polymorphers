...
public class HostGameController : MonoBehaviour, IObserver {
    ...
    private int clickJDailyDouble; //0,1,2
    private int clickDdailyDouble; //0,1,2
    ...
    void Start()
    {
        ...
        ISubject sub = selectTeamObject.GetComponent<ChooseTeamController>();
        sub.Attach(this);
        IObserver obs = audienceObject.GetComponent<AudienceController>();
        sub.Attach(obs);
    ...
    public void QuestionButtonClick()
    {
        ...
        if (currentButtonName[6] == 'D')
        {
            ...
            if (clickDdailyDouble == 0 && isDaily)
            {
